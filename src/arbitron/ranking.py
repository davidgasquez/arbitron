...
def calculate_bradley_terry_scores(
    ...
    total_games = [[win_matrix[i][j] + win_matrix[j][i] for j in range(n)] for i in range(n)]
        ...
        for i in range(n):
            ...
            for j in range(n):
                if i != j and total_games[i][j] > 0:
                    ...
                    strength_ratio = math.exp(old_log_strengths[i] - old_log_strengths[j])
                    numerator += wins_i
                    denominator += total_ij * (strength_ratio / (1 + strength_ratio))
            
            # Update log strength
            if denominator > 0:
                log_strengths[i] = math.log(numerator / denominator)
